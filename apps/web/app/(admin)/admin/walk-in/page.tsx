...
/**
 * Staff walk-in booking (Design D6, PRD A2.2). No OTP — the booking is
 * created with `verifiedVia: ADM_OVERRIDE` server-side, auditable. Member is
 * either quick-created by phone (primary path here; full admin member
 * search is M10.10) or attached to an existing member by raw id.
 *
 * Reuses the M10.3 `AvailabilityPicker` (date/start/slot-count grid + server
 * price preview) — this page only adds branch/court selection above it and
...
export default function AdminWalkInPage() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="text-lg font-semibold text-fg">สร้างการจองหน้างาน / Walk-in booking</h1>
        <p className="text-xs text-fg-muted">
          สำหรับลูกค้าที่มาจองหน้างานโดยไม่ผ่าน OTP / For customers booking on-site, without OTP.
        </p>
      </div>
